...
async def search_products_structured(
    ...
    try:
        ...
        logger.info(f"SEARCH_REQUEST | query='{search_query}' | original_query='{base_query}' | has_image={image is not None} | max_budget={max_budget}")
        
        # Get AI explanation from RAG pipeline
        # Pass image_path if you want the pipeline to specifically handle it (e.g. for context), 
        # but since we already extracted text, we can just use the augmented query.
        # However, passing it allows for potential future multimodal features.
        ...
        # Format and filter the results by budget
        products = []
        for i, product in enumerate(results):
        ...
        # Log results
        logger.info(f"SEARCH_RESULTS | query='{search_query}' | count={len(products)} | budget_applied={max_budget is not None}")
...
async def get_all_products(page: int = 1, limit: int = 12, session_id: Optional[str] = None):
    ...
    If session_id provided & history exists -> returns personalized adaptive feed (Hybrid Search).
    ...
    try:
        ...
        user_context_query = ""
        print(f"DEBUG: Fetching feed for session_id={session_id}")
        
        if session_id:
            user_context_query = user_tracker.get_cumulative_context(session_id, limit=20)
            print(f"DEBUG: Cumulative Context='{user_context_query}'")

        products = []
        
        if user_context_query:
            # PERSONALIZED FEED
            logger.info(f"FETCH_FEED | mode=personalized | session={session_id} | context='{user_context_query}'")
            print(f"DEBUG: Executing Personalized Search...")
            results = hybrid_searcher.search(user_context_query, limit=limit, offset=offset)
            print(f"DEBUG: Found {len(results)} personalized results")
            
            for i, product in enumerate(results):
                products.append({
                    "id": offset + i, # Virtual ID for frontend keys
                    "category": product.get("category", "Unknown"),
                    "rating": product.get("rating", 0),
                    "actual_price": product.get("actual_price", 0),
                    "discounted_price": product.get("discounted_price", 0),
            ...
            # GENERIC FEED (Fall back to DB order)
